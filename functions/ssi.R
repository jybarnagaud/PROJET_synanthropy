...
#       hullCoord <- tabularaster::cellnumbers(raster::raster(ras), sp_hull) %>%
#         rename(Cell = "cell_")
...
#       for(i in 1:sim) {
# 
#         cat(paste0("Simulation ", as.numeric(i), "/", sim, "\r"))
# 
#         
#         sp_resampled <- slice_sample(kernel_weights, n = spEvaluated %>% filter(Species == sp) %>% pull(nCellsPresent),
#                                      replace = FALSE, weight_by = weight)
# 
...
#           rstatix::cohens_d(Resolution ~ variable) %>% # equal = TRUE ?
#           mutate(Species = sp,
#                  Run = run,
#                  Resolution = value)
...
#  }
  ...
  for(value in resolution) { 
    ...
    # Extract naturalness raster values associated with each cell
    ras.value <- data.frame(value = terra::values(ras)) 
    ras.value$Cell <- 1:nrow(ras.value)
    colnames(ras.value)[1] = "value"
    # ras.value %<>% 
    #   mutate(Cell = as.numeric(rownames(ras.value))) %>% 
    #   # remove cells without naturalness values (e.g. offshore cells)
    #   na.omit() 
    
    ras.value <- ras.value[which(ras.value$value > 0), ]
    ras.value <- ras.value[which(!is.na(ras.value$value)), ]
    ...
    nullFull <- parallel::mclapply(unique(spEvaluated$Species), function(sp) {
      # prompt species evaluated
      cat(paste(Sys.time(), sp))
      cat(paste0(" (", spEvaluated %>% filter(Species == sp) %>% 
                   select(spNum), "/", max(spEvaluated$spNum), ")\n"))
    ...
    nullFull <- do.call(rbind.data.frame, nullFull)
    ...
    # create a list for all 3 tables
    results <- list("speciesScores" = data.frame(speciesScores), 
                    "effSizes"      = data.frame(effSizes), 
                    "samplesList"   = data.frame(samplesList))
